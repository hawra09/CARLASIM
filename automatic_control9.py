...
class CollisionSensor(object):
    ...
    def _on_collision(weak_self, event):
        ...
        if 'walker.pedestrian' in full_type_id:
            ...
            self.hud.notification('Collision with PEDESTRIAN', seconds=4.0)
...
def game_loop(args):
    ...
    try:
        ...
        try:
            ...
            print(f"[ERROR] World creation failed: {e}")
            world = None
            return
        controller = KeyboardControl(world)

        walker_bp = world.world.get_blueprint_library().filter('walker.pedestrian.*')
        controller_bp = world.world.get_blueprint_library().find('controller.ai.walker')


        spawn_points = world.map.get_spawn_points()
        random.shuffle(spawn_points)

        num_pedestrians = 5 # You can change this number
        for i in range(num_pedestrians):
            if i >= len(spawn_points):
                break

            spawn_loc = spawn_points[i].location
            spawn_transform = carla.Transform(spawn_loc + carla.Location(z=0.2))

            walker = world.world.try_spawn_actor(random.choice(walker_bp), spawn_transform)

            if walker:
                controller_actor = world.world.try_spawn_actor(controller_bp, carla.Transform(), attach_to=walker)
                ...
                if controller_actor:
                    ...
                    controller_actor.go_to_location(dest)
                    pedestrian_destinations[walker.id] = dest
                    controller_actor.set_max_speed(1 + random.random())

                    walkers.append(walker)
                    walker_controllers.append(controller_actor)
        ...
        num_vehicles = 1 # Number of background vehicles
        ...
        while True:
            ...
            for i, controller_actor in enumerate(walker_controllers):
                walker = walkers[i]
                walker_loc = walker.get_location()
                dest = pedestrian_destinations.get(walker.id)

                if dest is None or walker_loc.distance(dest) < 1.0:
                    ...
                    controller_actor.go_to_location(new_dest)
                    pedestrian_destinations[walker.id] = new_dest
                    print(f"[PEDESTRIAN] Reassigned destination for {walker.id}: {new_dest}")
